...
import edu.wm.cs.cs301.matthewcheng.generation.BSPBranch;
import edu.wm.cs.cs301.matthewcheng.generation.BSPLeaf;
import edu.wm.cs.cs301.matthewcheng.generation.BSPNode;
import edu.wm.cs.cs301.matthewcheng.generation.Floorplan;
import edu.wm.cs.cs301.matthewcheng.generation.Wall;
...
public class FirstPersonView {
	...
	private int traverseNodeCounter, traverseWallSectorCounter, drawRectCounter, drawRectLateCounter, drawRectWallCounter;
	 ...
	 * @param mazepanel the MazePanel in charge of drawing
	 */
	public FirstPersonView(int width, int height, int mapUnit, int stepSize, Floorplan seenWalls, BSPNode bspRoot, MazePanel mazepanel) {
		...
		color = setColor(Container.getInstance().getMaze().getSeed());
		...
		// initialize fields
		rSet = new RangeSet();
		// THIS IS NEW AS OF P5
		mp = mazepanel;
	}

	private int setColor(int seed) {
		switch (seed % 6) {
			case 0:
				return Color.BLUE;
			case 1:
