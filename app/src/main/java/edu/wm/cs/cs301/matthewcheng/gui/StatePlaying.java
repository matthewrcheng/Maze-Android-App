...
public class StatePlaying extends DefaultState {
    ...
    public boolean keyDown(UserInput key, int value) {
        ...
        case UP: // move forward
            ...
            if (isOutside(px,py)) {
                // THIS IS WHERE WE SWITCH TO WINNING
                control.switchFromPlayingToWinning();
            }
            break;
        case LEFT: // turn left
            rotate(1);
        ...
        case RETURNTOTITLE: // escape to title screen
            // WE JUST NEED TO PERFORM A BACK BUTTON PRESS
    ...
    public void setShowMaze(boolean mode) {
        showMaze = mode;
    }

    public void setShowSolution(boolean mode) {
        showSolution = mode;
    }
    ...
    private void drawHintIfNecessary() {
    	...
    	// for guidance
    	if (isFacingDeadEnd() & control.isManual()) {
